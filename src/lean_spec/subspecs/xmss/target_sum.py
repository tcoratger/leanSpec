...
from .constants import TARGET_SUM
from .message_hash import apply as apply_message_hash
from .structures import Parameter, Randomness


def encode(
    parameter: Parameter, message: bytes, rho: Randomness, epoch: int
) -> Optional[List[int]]:
    """
    ...
    codeword_candidate = apply_message_hash(parameter, epoch, rho, message)

    # Check if the candidate satisfies the target sum condition.
    if sum(codeword_candidate) == TARGET_SUM:
