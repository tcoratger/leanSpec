...
def test_constructor_errors() -> None:
    ...
    with pytest.raises(SSZSelectorError, match="out of range"):
    ...
    with pytest.raises(SSZTypeCoercionError, match="Expected None"):
...
def test_pydantic_validation_errors() -> None:
    ...
    with pytest.raises((ValidationError, SSZSelectorError)):
        model(v={"selector": 9, "value": 0})

    # Invalid value for None option in model context
    with pytest.raises((ValidationError, SSZTypeCoercionError)):
...
def test_deserialize_errors() -> None:
    ...
    # Too small scope
    with pytest.raises(SSZDecodeError, match="scope too small"):
    ...
    with pytest.raises(SSZDecodeError, match="no payload bytes"):
...
def test_get_byte_length_raises() -> None:
    ...
    with pytest.raises(SSZTypeDefinitionError, match="variable-size"):
...
def test_union_type_validation() -> None:
    ...
    with pytest.raises(SSZTypeDefinitionError, match="None at index 0"):
    ...
    with pytest.raises(SSZTypeCoercionError):
...
def test_union_boundary_cases() -> None:
    ...
    with pytest.raises(SSZTypeDefinitionError, match="only option"):
