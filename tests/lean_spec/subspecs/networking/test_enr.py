...
from lean_spec.subspecs.networking.enr import ENR, EnrKey, Eth2Data
...
class TestEnrKey:
    """Tests for ENR key enumeration."""

    def test_identity_keys(self) -> None:
        """Identity keys have correct values."""
        assert EnrKey.ID.value == "id"
        assert EnrKey.SECP256K1.value == "secp256k1"

    def test_network_keys(self) -> None:
        """Network keys have correct values."""
        assert EnrKey.IP.value == "ip"
        assert EnrKey.IP6.value == "ip6"
        assert EnrKey.TCP.value == "tcp"
        assert EnrKey.UDP.value == "udp"

    def test_ethereum_keys(self) -> None:
        """Ethereum-specific keys have correct values."""
        assert EnrKey.ETH2.value == "eth2"
        assert EnrKey.ATTNETS.value == "attnets"
        assert EnrKey.SYNCNETS.value == "syncnets"

    def test_key_string_conversion(self) -> None:
        """Keys convert to their string values."""
        assert str(EnrKey.ID) == "id"
        assert str(EnrKey.ETH2) == "eth2"
...
class TestENR:
    ...
    def test_enr_has_key(self) -> None:
        ...
        assert enr.has(EnrKey.ID)
        assert enr.has(EnrKey.SECP256K1)
        assert not enr.has(EnrKey.IP)
        assert not enr.has(EnrKey.ETH2)
    ...
    def test_enr_get_key(self) -> None:
        ...
        assert enr.get(EnrKey.ID) == b"v4"
        assert enr.get(EnrKey.IP) is None
