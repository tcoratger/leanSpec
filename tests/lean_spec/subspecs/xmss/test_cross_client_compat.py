...
# def _assert_verify_or_xfail(result: bool, client_name: str) -> None:
#     """Treat known prod-vector mismatch as expected until config alignment lands."""
#     if not result:
#         pytest.xfail(
#             f"{client_name} cross-client verification mismatch with current prod XMSS parameters"
#         )


# def _assert_signature_len_or_xfail(raw: bytes, client_name: str) -> None:
#     """Treat known prod-vector length mismatch as expected until config alignment lands."""
#     if len(raw) != PROD_CONFIG.SIGNATURE_LEN_BYTES:
#         pytest.xfail(
#             f"{client_name} signature length {len(raw)} does not match current prod "
#             f"config length {PROD_CONFIG.SIGNATURE_LEN_BYTES}"
#         )
...
#         _assert_signature_len_or_xfail(raw, client_name="ream")
...
#         _assert_verify_or_xfail(
#             PROD_SIGNATURE_SCHEME.verify(
#                 ream_public_key,
#                 Slot(REAM_EPOCH),
#                 REAM_MESSAGE,
...
#         _assert_signature_len_or_xfail(raw, client_name="zeam")
