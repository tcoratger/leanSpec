...
from lean_spec.subspecs.xmss.prf import apply, key_gen
...
def test_key_gen_is_random() -> None:
    ...
    num_trials = 10
    keys = {key_gen() for _ in range(num_trials)}
...
def test_apply_is_sensitive_to_inputs() -> None:
    ...
    epoch1 = 10
    chain_index1 = 20
    baseline_output = apply(key1, epoch1, chain_index1)
    assert len(baseline_output) == HASH_LEN_FE

    # Test sensitivity to the key.
    key2 = b"\x22" * PRF_KEY_LENGTH
    output_key_changed = apply(key2, epoch1, chain_index1)
    assert baseline_output != output_key_changed

    # Test sensitivity to the epoch.
    epoch2 = 11
    output_epoch_changed = apply(key1, epoch2, chain_index1)
    assert baseline_output != output_epoch_changed

    # Test sensitivity to the chain_index.
    chain_index2 = 21
    output_index_changed = apply(key1, epoch1, chain_index2)
