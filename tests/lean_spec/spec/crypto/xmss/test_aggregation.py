...
def test_type_one_verify_rejects_public_key_count_mismatch(key_manager: XmssKeyManager) -> None:
    """Type-1 verification refuses a public_key set that does not match the bitfield."""
...
def test_type_two_split_by_message_rejected_under_test_prover(key_manager: XmssKeyManager) -> None:
...
def test_type_two_aggregate_rejects_mismatched_public_key_layout(
    key_manager: XmssKeyManager,
) -> None:
...
def test_type_two_aggregate_propagates_prover_error(key_manager: XmssKeyManager) -> None:
    ...
    with pytest.raises(AggregationError, match="merge_many_type_1 failed"):
        TypeTwoMultiSignature.aggregate(parts=[part], public_keys_per_part=[public_keys])
