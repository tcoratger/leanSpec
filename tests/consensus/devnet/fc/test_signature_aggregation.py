...
def test_single_attestation_in_block_body(
    fork_choice_test: ForkChoiceTestFiller,
) -> None:
    """
    Single attestation results in one aggregated attestation in block body.

    Scenario
    --------
    Block at slot 2 includes attestation from validators 0 and 3 targeting block 1.

    Expected
    --------
    - 1 aggregated attestation in block body
    - Covers validators {0, 3}
    """
    fork_choice_test(
        steps=[
            BlockStep(
...
def test_full_attestation_pipeline_across_three_blocks(
    fork_choice_test: ForkChoiceTestFiller,
) -> None:
    """
    Complete signature aggregation pipeline across multiple blocks.

    This test demonstrates the following flow:
    1. Block 1: No body attestations (first block after genesis)
    2. Block 2: Includes attestations for block 1
    3. Block 3: Includes attestations for block 2

    Each block:
    - Contains attestations from validators voting on the previous block
    - Proposer's own attestation becomes available for next block

    This is how attestations flow in a real chain.
    """
    fork_choice_test(
        steps=[
            # Block 1: First block, no attestations to include yet
            BlockStep(
                block=BlockSpec(slot=Slot(1), label="block_1"),
                checks=StoreChecks(
                    head_slot=Slot(1),
            ...
            # Block 2: Include attestations from validators voting for block 1
            BlockStep(
                block=BlockSpec(
                    slot=Slot(2),
                    label="block_2",
            ...
            # Block 3: Include attestations from validators voting for block 2
            BlockStep(
                block=BlockSpec(
                    slot=Slot(3),
                    attestations=[
...
def test_attestations_accumulate_across_chain(
    fork_choice_test: ForkChoiceTestFiller,
) -> None:
    """
    Attestations accumulate as the chain grows.

    Scenario
    --------
    Four-block chain where each block includes more attestations:
    - Block 1: 0 attestations
    - Block 2: 1 attestation (2 validators for block 1)
    - Block 3: 1 attestation (3 validators for block 2)
    - Block 4: 1 attestation (all 4 validators for block 3)

    This demonstrates aggregation scaling with validator participation.
    """
    fork_choice_test(
        steps=[
            BlockStep(
